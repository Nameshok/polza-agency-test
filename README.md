...
| `npm run emails` | вывести адреса из доменов и прогнать по шести этапам валидации (`-- --smtp` включает SMTP) |
| `npm test` | 25 тестов: нормализация, CSV-парсер, валидация адресов, SMTP; базы не требуют |
...
scripts/
  ...
  enrich_emails.ts   критерий «валидные email»: вывод адресов из доменов + шесть этапов
  ...
  lib/email.ts       валидация адреса без сети: опечатки, синтаксис, ролевые, одноразовые
  lib/smtp.ts        SMTP-сессия: вопрос про ящик и распознавание catch-all
  normalize.test.ts  тесты нормализации на реальных значениях из выгрузки
  email.test.ts      тесты валидации адресов, включая «ролевой не бывает valid»
  smtp.test.ts       тесты SMTP против локального поддельного сервера
...
- **`email_candidates`** — адреса, выведенные из доменов, со статусом и этапом, на котором
  адрес выбыл. Колонка `source` существует затем, чтобы догадка никогда не смешалась
  с адресом, полученным из источника.
...
`docs/как-проверял-подробно.md`. Живой вывод команд, снятый с чистой базы:
`docs/run-output.txt` — тесты, `typecheck`, загрузка, повторная загрузка, проверки
на живой базе, прогон валидации адресов и `build`; `docs/report-output.txt` — отчёт
по данным, три запроса из задания и **сверка каждого числа из `ANOMALIES.md` с базой**.

Последнее стоит отдельного слова. Документ, в котором числа названы, но не пересчитаны, —
это утверждения о работе, а не работа. Поэтому `npm run report` в конце печатает таблицу
«показатель / значение в базе / значение в документе»: любое расхождение видно сразу
и без чтения текста.
...
- **Поля email в выгрузке нет,** поэтому адреса выведены из доменов сайтов, а не получены
  из источника — и помечены `source = 'derived_from_site'`. Валидных ноль: 877 доменов
  из 883 не существуют в DNS. Разбор и цифры — `ANOMALIES.md`, п. 10.
- **SMTP-этап по умолчанию выключен.** Живая проверка с произвольного IP портит репутацию
  отправителя; включается флагом `npm run emails -- --smtp`. Сам код SMTP покрыт тестами
  против локального поддельного сервера — включая случай «сервер молча закрыл соединение»,
  на котором он раньше вис навсегда. **Против живых чужих серверов я его не гонял
  намеренно:** на этих данных проверять нечего (доменов нет), а слать незапрошенные
  RCPT-запросы посторонним ради теста неправильно.
