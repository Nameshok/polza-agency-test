...
Поиск «Медиа» с фильтром по Санкт-Петербургу. Третья строка снизу, «ООО «Темп Медиа»»
(`c_001128`) — это та запись, которая в CSV лежала с битой кодировкой как
`РћРћРћ В«РўРµРјРї РњРµРґРёР°В»`: после ремонта она нормально ищется:
